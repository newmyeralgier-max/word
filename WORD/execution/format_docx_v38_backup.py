...
def _format_formula(p):
    ...
    ★ v4.0 PRIMUM NON NOCERE: НЕ перезаписываем runs — только ставим выравнивание и tab stops.
    Для OMML-формул: тело в m:oMath, runs содержат номер — НЕ трогаем.
    Для текстовых формул: тоже НЕ перезаписываем runs — tab stops выровняют визуально."""
    
    # ★ v4.0: НЕ трогаем runs вообще! Только визуальное выравнивание через tab stops.
...
def _handle_table_caption(p, doc=None, legacy=False):
    ...
    ★ v4.0 PRIMUM NON NOCERE: НЕ удаляем следующий параграф, НЕ перезаписываем runs.
    Только стилизация: выравнивание, шрифт, отступы."""
...
def process_document(input_path, output_path=None, fast=False, legacy=False):
    ...
    # #v4_REMOVED: ФИКС I — удаление серий пустых абзацев
    # PRIMUM NON NOCERE: пустые абзацы НЕ удаляются — Word сожмёт при рендеринге
    # Оставлено как комментарий для истории:
    ...
    # #v4_REMOVED: ФИКС 14 — удаление хвостовых параграфов после ПРИЛОЖЕНИЯ
    # PRIMUM NON NOCERE: не удаляем параграфы в хвосте документа
