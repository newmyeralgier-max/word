...
python pipeline/run_all.py \
    ...
    --output "WORD/data/Готово/Уч пособ Матлаб ПРАВИЛЬНО3_GOST.docx"
```

Сохранит бэкап оригинала в `WORD/data/backups/<имя>_pre_run.docx`, прогонит 12 шагов по порядку, выпишет результат.
...
5. Скрипт **не должен** ничего удалять из `WORD/data/`, кроме создания бэкапа в `WORD/data/backups/`.

## Markdown → DOCX

Для сборки .docx из Markdown-черновика или дописки MD-фрагмента в существующий файл — см. отдельный модуль [`md_to_docx/README.md`](md_to_docx/README.md).
...
- **Не править** `WORD/data/*.docx` напрямую без бэкапа в `WORD/data/backups/`.
